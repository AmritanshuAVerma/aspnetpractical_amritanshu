...
namespace Album.Data.Repositories
{
    public class AlbumRepository : BaseReporitory<Album.Data.Models.Album>, IAlbumRepository
    {
        ...
        public AlbumRepository(AlbumContext albumContext) : base(albumContext)
        {
        ...
        {
            return await albumContext.Album.AsNoTracking()
        ...
        public async Task<bool> UpdateAlbumName(Models.Album album, CancellationToken cancellationToken = default)
        {
            ...
            var data = albumContext.Album.FirstOrDefault(x => x.AlbumId == album.AlbumId);
            if (data != null) { 
                data.Title = album.Title;
                albumContext.Album.Update(data);
                var result = await albumContext.SaveChangesAsync();
                return true;
            }
            return false;
        }      
